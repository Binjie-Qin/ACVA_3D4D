...
function [RES,RES0,NUM]=ACVA_3D4D(rima,wid,step,level,seed)
if (exist('level') ~= 1)
    level='n';
end
if (exist('seed') ~= 1)
...
range_h = 1:step:(H-wid+1);
range_k = 1:step0:(K-wid0+1);
...
if (range_h(end)~=(H-wid+1))
    range_h = [range_h (H-wid+1)];
end

if (range_k(end)~=(K-wid0+1))
    range_k = [range_k (K-wid0+1)];
end
disp('Denoising(%)');
totalnum=length(range_y)*length(range_x)*length(range_h)*length(range_k)/100.00;
countnum=0;
for  x=range_x%
    ...
    for y=range_y%
        ...
        for h=range_h%
            ...
            for k=range_k%
                ...
                tic
                [~,cres0,num0]=CMPLRA_3D4D(rima(x:x+wid-1,y:y+wid-1,h:h+wid-1,k:k+wid0-1),level,seed);
                toc
                RES0(x:x+wid-1,y:y+wid-1,h:h+wid-1,k:k+wid0-1)=RES0(x:x+wid-1,y:y+wid-1,h:h+wid-1,k:k+wid0-1)+cres0;
                NUM(x:x+wid-1,y:y+wid-1,h:h+wid-1,k:k+wid0-1)=NUM(x:x+wid-1,y:y+wid-1,h:h+wid-1,k:k+wid0-1)+num0;
...
function [Y1,Y10,num0]= CMPLRA_3D4D(rima,level,seed)
...
Volume=s(1)*s(2)*s(3)*s(4);
X=zeros(d*d*d*s(4),(s(1)-d+1)*(s(2)-d+1)*(s(3)-d+1));
for i=1:s(4)

    X(d*d*d*(i-1)+1:d*d*d*i,:)=extract3D(data(:,:,:,i),d);
...
k44=max(floor(Volume/(256*256)),4);
...
knm=zeros(k44,1);
for i = 1:k44
    knm(i)=max(floor(length(indexes4{i})/M),1);
...
[Y1,Y10,num0]=comb4Dw(denoi_stacked1,d,s);
